...
function renderValidation() {
  ...
  const winterOption = { plannedProduction: 70000, milkTonnes: 3.5, milkPrice: 20000, milkYield: 20000, salesRequest: 70000, marketInvestment: 5000, machineCapacityUsed: 0, machineSlotsUsed: 0, newMachinePrice: 35000, newMachineCapacity: 72000, newMachineMaintenance: 1800, newMachineLife: 8, newMachineSlots: 1, newBorrowing: 50000, newLoanTerm: 8, newLoanInterestRate: 10, premises: [{ name: "D", active: true, slots: 1, rent: 17000, transport: .1, production: 70000 }] };
  const result = calculate(winterCompany, winterAssumptions, winterOption, 20000);
  const passed = result.netProfit === -75175 && result.closingCash === 37950 && result.taxLossPoolEnd === 75175;
  return `<section class="card section" data-testid="year1-validation"><div class="section-title-row"><div><h2>Year 1 calculation check</h2><p class="small">Independent recorded Winter season — this must pass before the tool is deployed.</p></div><div class="notice ${passed ? "success" : "danger"}"><strong data-testid="validation-status">${passed ? "PASS" : "FAIL"}</strong><br>${passed ? "Matches the classroom result." : "Trace and correct the formula before deployment."}</div></div>
    <div class="check-grid"><div class="check-result"><span class="metric-label">Recorded inputs</span><p class="small">Opening cash Sh 100,000 · M1 purchase Sh 35,000 · 3.5t milk at Sh 20,000/t · 70,000 produced/requested · 20,000 allocated sales · D rent Sh 17,000 · 10% loan/season.</p></div>
    <div class="grid three"><div class="metric"><span class="metric-label">Net loss</span><span class="metric-value ${result.netProfit === -75175 ? "positive" : "negative"}">${signedMoney(result.netProfit)}<br><span class="small">Expected −Sh 75,175</span></span></div><div class="metric"><span class="metric-label">Closing cash</span><span class="metric-value ${result.closingCash === 37950 ? "positive" : "negative"}">${money(result.closingCash)}<br><span class="small">Expected Sh 37,950</span></span></div><div class="metric"><span class="metric-label">Tax-loss pool added</span><span class="metric-value ${result.taxLossPoolEnd === 75175 ? "positive" : "negative"}">${money(result.taxLossPoolEnd)}<br><span class="small">Expected Sh 75,175</span></span></div></div></div>
    <p class="field-note">Trace: revenue ${money(result.revenue)} − milk ${money(result.milkCost)} − maintenance ${money(result.maintenance)} − depreciation ${money(result.depreciation)} = gross loss ${signedMoney(result.grossProfit)}; then transport, market, salaries, rent, and interest produce the expected loss.</p>
  </section>`;
}
...
function renderRecommendation(results, lowResults) {
  ...
  return `<section class="card section recommendation"><div class="section-title-row"><div><h2>Recommendation</h2><p class="small">Draft logic favours cash survival over theoretical profit.</p></div><span class="estimate-label">Editable draft</span></div>
    ...
    <div class="notice ${complete ? "" : "danger"}"><strong>${complete ? "Most important assumption:" : "Status:"}</strong> ${esc(complete ? "Actual sales allocation is uncertain; the company previously received 20,000 sales from a 70,000-unit request." : pick.text)}</div>
